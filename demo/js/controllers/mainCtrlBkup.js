...
app.controller('MainController', function($scope, $rootScope, RemoteDataSrvc, $location, $q){
...
console.log('ive updated');
  ...
  // $scope.s01 = RemoteDataSrvc.getData();
  $scope.myServers = RemoteDataSrvc.getData('monitorAuth?authToken=1234').then(function(response) {
    ...
    angular.forEach($scope.serverList, function(oneServer) {
      console.log('monitorFolder/' + oneServer);
      // $scope.servers.push(oneServer);
